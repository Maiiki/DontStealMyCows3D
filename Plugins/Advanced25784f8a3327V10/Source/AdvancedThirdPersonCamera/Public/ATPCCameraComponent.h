...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "ATPC|Settings")
	TArray<TObjectPtr<UATPCCameraModeDataAsset>> CameraModesAssets;

	/** Storage for instanced camera modes sorted by FGameplayTag */
	UPROPERTY(Transient)
	TMap<FGameplayTag, TObjectPtr<UATPCCameraModeDataAsset>> SortedCameraModes;

	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraModeDataAsset> CustomCameraMode;
	...
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraLocationObject> LocationObject;
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraFOVObject> FOVObject;
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraFadingObject> FadingObject;
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraFollowTerrainObject> FollowTerrainObject;
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraShakesObject> CameraShakesObject;
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraLockOnTargetObject> LockOnTargetObject;

	UPROPERTY(Transient)
	TArray<TObjectPtr<UATPCCameraBaseObject>> CameraObjList;

private:
	UPROPERTY(Transient)
	TObjectPtr<UATPCCameraModeDataAsset> EmptyCameraMode;
